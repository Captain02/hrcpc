<template>
  <div class="mobile-container">
    <h1 class="register-message">加入失败，您尚未注册，请填写相关信息进行注册，注册成功后可在百团争鸣小程序中查看更多详情</h1>
    <el-form class="register-form" :model="user" :rules="rules" ref="userForm" size="small">
    ...
    handleClick() {
      this.$refs['userForm'].validate((valid) => {
         if (valid) {
          ...
          registerApi(this.corid, this.openid, this.user.username, this.user.name, this.user.gender, this.user.password, collegeName, collegetieName, this.user.email, this.user.mobile, this.user.wechart, this.user.qq, this.user.descs).then((result) => {
            // console.log(result)
            this.$router.replace({
              path: '/result',
              query: {
